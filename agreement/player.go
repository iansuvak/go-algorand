...
func (p *player) enterRound(r routerHandle, source event, target round) []action {
	...
	if e.t() == payloadPipelined {
		...
		msg := message{MessageHandle: nil, Tag: protocol.ProposalPayloadTag, UnauthenticatedProposal: e.UnauthenticatedPayload} // TODO do we want to keep around the original handle?
